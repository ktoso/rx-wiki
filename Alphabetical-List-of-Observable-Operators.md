...
* **`apply( )`** (scala) — _see `create( )`_
* **`asObservable( )`** (kotlin) — _see `from( )` (et al.)_
...
* **`blocking( )`** (clojure) — _see `toBlockingObservable( )`_
...
* **`catch( )`** (clojure) — _see `onErrorResumeNext( )`_
...
* **`drop( )`** (scala/clojure) — _see `skip( )`_
* **`dropRight( )`** (scala) — _see `skipLast( )`_
* **`dropUntil( )`** (scala) — _see `skipUntil( )`_
* **`dropWhile( )`** (scala) — _see `skipWhile( )`_
* **`drop-while( )`** (clojure) — _see `skipWhile( )`_
...
* **`finally( )`** (clojure) — _see `finallyDo( )`_
...
* **`firstOrElse( )`** (scala) — _see `firstOrDefault( )`_
...
* **`generator( )`** (clojure) — _see `generate( )`_
...
* **`group-by( )`** (clojure) — _see `groupBy( )`_
...
* **`merge-delay-error( )`** (clojure) — _see `mergeDelayError( )`_
...
* **`nth( )`** (clojure) — _see `elementAt( )` and `elementAtOrDefault( )`_
...
* **`reductions( )`** (clojure) — _see `scan( )`_
...
* **`rest( )`** (clojure) — _see `next( )`_
* **`return( )`** (clojure) — _see `just( )`_
...
* **`switch( )`** (scala) — _see `switchOnNext( )`_
...
* **`takeRight( )`** (scala) — _see `last( )` or `takeLast( )`_
...
* **`take-while( )`** (clojure) — _see `takeWhile( )`_
...
* **`toBlocking( )`**] - _see `toBlockingObservable( )`_
