...
* **`blocking( )`** — _see `toBlockingObservable( )`_ (clojure)
...
* **`catch( )`** — _see `onErrorResumeNext( )`_ (clojure)
...
* **`dropRight( )`** — _see `skipLast( )`_ (scala)
* **`dropUntil( )`** — _see `skipUntil( )`_ (scala)
* **`dropWhile( )`** — _see `skipWhile( )`_ (scala)
* **`drop-while( )`** — _see `skipWhile( )`_ (clojure)
...
* **`finally( )`** — _see `finallyDo( )`_ (clojure)
...
* **`firstOrElse( )`** — _see `firstOrDefault( )`_ (scala)
...
* **`flatten( )`** — _see `merge( )`_ (scala)
* **`flattenDelayError( )`** — _see `mergeDelayError( )`_ (scala)
* **`foldLeft( )`** — _see `reduce( )`_ (scala)
* **`forall( )`** — _see `all( )`_ (scala)
...
* **`fromFunc0( )`** — _see `fromCallable( )`_ (`rxjava-async`)
...
* **`generator( )`** — _see `generate( )`_ (clojure)
...
* **`group-by( )`** — _see `groupBy( )`_ (clojure)
...
* **`head( )`** — _see `first( )` (`BlockingObservable`)_ (scala)
* **`headOrElse( )`** — _see `firstOrDefault( )`_ (scala)
...
* **`merge-delay-error( )`** — _see `mergeDelayError( )`_ (clojure)
...
* **`seq( )`** — _see `getIterator( )`_ (clojure)
...
* **`singleOption( )`** — _see `singleOrDefault( )` (`BlockingObservable`)_ (scala)
...
* **`switch( )`** — _see `switchOnNext( )`_ (scala)
...
* **`take-while( )`** — _see `takeWhile( )`_ (clojure)
